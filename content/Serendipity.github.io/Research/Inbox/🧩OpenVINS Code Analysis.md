...
2. `state->_clones_IMU.size() < std::min(state->_options.max_clone_size, 5)` → 그냥 return하고 종료
	1. 이유는 그냥 넉넉히 5개가 있어야 triangulation을 할 수 있으니까 그런 것 같다.
3. 
### try_to_initialize
...
4. [[#perform_matching]]을 parallel하게 돌려서 왼/오 이미지 각각 temporal opticalflow tracking. VINS-Fusion과는 다르게 같은 시간 left to right tracking은 하지 않음.
...
5. bias 구하기
	1. `bg = w_avg_2to1;`
	2. [ ] `ba = a_avg_2to1 - quat_2_Rot(q_GtoI) * gravity_inG` #점검 
